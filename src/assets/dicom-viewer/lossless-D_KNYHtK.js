...
var y = Object.defineProperty, D = (a, t) => {
  ...
  read(a) {
    ...
    for (let n = 1; n <= this.numComp; n += 1) {
      ...
      e = a.get8(), t += 1, this.components[s] || (this.components[s] = { ...L }), this.components[s].hSamp = e >> 4, this.components[s].vSamp = e & 15, this.components[s].quantTableSel = a.get8(), t += 1;
    }
    if (t !== r)
...
var w = (...a) => {
  ...
  read(t, e) {
    ...
    for (r += 2; r < f; ) {
      ...
      for (this.th[s] = 1, this.tc[s][o] = 1, i = 0; i < 16; i += 1)
        this.l[s][o][i] = t.get8(), r += 1;
      for (i = 0; i < 16; i += 1)
        for (c = 0; c < this.l[s][o][i]; c += 1) {
          if (r > f)
            throw new Error("ERROR: Huffman table format error [count>Lh]");
          this.v[s][o][i][c] = t.get8(), r += 1;
        }
    }
    if (r !== f)
    ...
    for (i = 0; i < 4; i += 1)
      for (c = 0; c < 2; c += 1)
        this.tc[i][c] !== 0 && this.buildHuffTable(e[i][c], this.l[i][c], this.v[i][c]);
    return 1;
  }
  //	Build_HuffTab()
...
}, h(R, "MSB", 2147483648), R), p, B = (p = class {
  constructor() {
    h(this, "precision", []);
    // Quantization precision 8 or 16
    h(this, "tq", [0, 0, 0, 0]);
    // 1: this table is presented
    h(this, "quantTables", w(4, 64));
  }
  read(t, e) {
    let r = 0, n, s, o;
    const i = t.get16();
    for (r += 2; r < i; ) {
      ...
      if (this.tq[s] = 1, this.precision[s] === 8) {
        ...
        p.enhanceQuantizationTable(this.quantTables[s], e);
      } else {
        for (o = 0; o < 64; o += 1) {
          if (r > i)
            throw new Error("ERROR: Quantization table format error");
...
h(p, "enhanceQuantizationTable", function(t, e) {
  for (let r = 0; r < 8; r += 1)
  ...
  constructor(t, e) {
    ...
    h(this, "stream", null);
    h(this, "frame", new S());
    h(this, "huffTable", new C());
    h(this, "quantTable", new B());
    h(this, "scan", new P());
    h(this, "DU", w(10, 4, 64));
    // at most 10 data units in a MCU, at most 4 data units in one component
    h(this, "HuffTab", w(4, 2, 50 * 256));
    h(this, "IDCT_Source", []);
    h(this, "nBlock", []);
    ...
    h(this, "marker", 0);
    h(this, "markerIndex", 0);
    h(this, "numComp", 0);
    h(this, "restartInterval", 0);
    h(this, "selection", 0);
    h(this, "xDim", 0);
    h(this, "yDim", 0);
    h(this, "xLoc", 0);
    h(this, "yLoc", 0);
    h(this, "outputData", null);
    h(this, "restarting", !1);
    h(this, "mask", 0);
    h(this, "numBytes", 0);
    h(this, "precision");
  ...
  decode(t, e, r, n) {
    ...
    do {
      ...
      for (i = 0; i < this.numComp; i += 1)
        c = this.scan.components[i].scanCompSel, this.qTab[i] = this.quantTable.quantTables[this.components[c].quantTableSel], this.nBlock[i] = this.components[c].vSamp * this.components[c].hSamp, this.dcTab[i] = this.HuffTab[this.scan.components[i].dcTabSel][0], this.acTab[i] = this.HuffTab[this.scan.components[i].acTabSel][1];
      for (this.xDim = this.frame.dimX, this.yDim = this.frame.dimY, this.numBytes === 1 ? this.outputData = new Uint8Array(new ArrayBuffer(this.xDim * this.yDim * this.numBytes * this.numComp)) : this.outputData = new Uint16Array(new ArrayBuffer(this.xDim * this.yDim * this.numBytes * this.numComp)), s += 1; ; ) {
        for (f[0] = 0, g[0] = 0, i = 0; i < 10; i += 1)
          o[i] = 1 << this.precision - 1;
  ...
  decodeRGB(t, e, r) {
    ...
    let n, s, o, i, c, f, g;
    for (t[0] = this.selector(0), t[1] = this.selector(1), t[2] = this.selector(2), i = 0; i < this.numComp; i += 1)
      for (o = this.qTab[i], n = this.acTab[i], s = this.dcTab[i], c = 0; c < this.nBlock[i]; c += 1) {
        for (f = 0; f < this.IDCT_Source.length; f += 1)
          this.IDCT_Source[f] = 0;
        let m = this.getHuffmanValue(s, e, r);
        if (m >= 65280)
          return m;
        for (t[i] = this.IDCT_Source[0] = t[i] + this.getn(r, m, e, r), this.IDCT_Source[0] *= o[0], g = 1; g < 64; g += 1) {
  ...
  decodeSingle(t, e, r) {
    ...
    for (this.restarting ? (this.restarting = !1, t[0] = 1 << this.frame.precision - 1) : t[0] = this.selector(), s = 0; s < this.nBlock[0]; s += 1) {
      if (n = this.getHuffmanValue(this.dcTab[0], e, r), n >= 65280)
        return n;
      if (o = this.getn(t, n, e, r), i = o >> 8, i >= l.RESTART_MARKER_BEGIN && i <= l.RESTART_MARKER_END)
        return i;
      t[0] += o;
    }
    return 0;
  ...
  getHuffmanValue(t, e, r) {
    ...
    if (r[0] < 8 ? (e[0] <<= 8, s = this.stream.get8(), s === 255 && (this.marker = this.stream.get8(), this.marker !== 0 && (this.markerIndex = 9)), e[0] |= s) : r[0] -= 8, n = t[e[0] >> r[0]], (n & l.MSB) !== 0) {
      if (this.markerIndex !== 0)
        return this.markerIndex = 0, 65280 | this.marker;
      e[0] &= 65535 >> 16 - r[0], e[0] <<= 8, s = this.stream.get8(), s === 255 && (this.marker = this.stream.get8(), this.marker !== 0 && (this.markerIndex = 9)), e[0] |= s, n = t[(n & 255) * 256 + (e[0] >> r[0])], r[0] += 8;
    }
    if (r[0] += 8 - (n >> 8), r[0] < 0)
      throw new Error("index=" + r[0] + " temp=" + e[0] + " code=" + n + " in HuffmanValue()");
    return r[0] < this.markerIndex ? (this.markerIndex = 0, 65280 | this.marker) : (e[0] &= 65535 >> 16 - r[0], n & 255);
  }
  getn(t, e, r, n) {
    let s, o;
    if (this.stream === null)
    ...
    if (n[0] -= e, n[0] >= 0) {
      if (n[0] < this.markerIndex && !this.isLastPixel())
        return this.markerIndex = 0, (65280 | this.marker) << 8;
      s = r[0] >> n[0], r[0] &= 65535 >> 16 - n[0];
    } else {
      if (r[0] <<= 8, o = this.stream.get8(), o === 255 && (this.marker = this.stream.get8(), this.marker !== 0 && (this.markerIndex = 9)), r[0] |= o, n[0] += 8, n[0] < 0) {
        if (this.markerIndex !== 0)
          return this.markerIndex = 0, (65280 | this.marker) << 8;
        r[0] <<= 8, o = this.stream.get8(), o === 255 && (this.marker = this.stream.get8(), this.marker !== 0 && (this.markerIndex = 9)), r[0] |= o, n[0] += 8;
      }
      if (n[0] < 0)
        throw new Error("index=" + n[0] + " in getn()");
      if (n[0] < this.markerIndex)
        return this.markerIndex = 0, (65280 | this.marker) << 8;
      s = r[0] >> n[0], r[0] &= 65535 >> 16 - n[0];
    }
  ...
  setValue8(t, e) {
    ...
    d ? this.outputData[t] = e : this.outputData[t] = (e & 255) << 8 | e >> 8 & 255;
  }
  getValue8(t) {
  ...
  62,
  63
]), h(l, "MAX_HUFFMAN_SUBTREE", 50), h(l, "MSB", 2147483648), h(l, "RESTART_MARKER_BEGIN", 65488), h(l, "RESTART_MARKER_END", 65495), l);
export {
  L as ComponentSpec,
  v as DataStream,
  _ as Decoder,
